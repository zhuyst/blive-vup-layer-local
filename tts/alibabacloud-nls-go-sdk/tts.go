...
const (
	...
	TTS_RUN_NAME       = "RunSynthesis"
	TTS_COMPLETED_NAME = "StopSynthesis"
...
func onTtsStartHandler(isErr bool, text []byte, proto *nlsProto) {
	tts := checkTtsNlsProto(proto)
	...
	req.Header.Name = TTS_COMPLETED_NAME
	b, _ = json.Marshal(req)
	tts.nls.logger.Println("send:", string(b))
	tts.nls.cmd(string(b))
}
...
var ttsProto = commonProto{
	...
	handlers: map[string]func(bool, []byte, *nlsProto){
		...
		"SynthesisStarted":   onTtsStartHandler,
		RAW_HANDLER:          onTtsRawResultHandler,
		"SynthesisCompleted": onTtsCompletedHandler,
